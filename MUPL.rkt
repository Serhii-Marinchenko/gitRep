;; Implementing mupl (a Made Up Programming Language).
  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([sub1 (eval-under-env (call-funexp e) env)])
           (if (closure? sub1)
               (letrec ([body (fun-body (closure-fun sub1))]
                        [env (closure-env sub1)]
                        [fname (fun-nameopt (closure-fun sub1))]
                        [arg-name (fun-formal (closure-fun sub1))]
                        [sub2 (eval-under-env (call-actual e) env)])
                 (eval-under-env body (if fname
                                          (cons (cons arg-name sub2) (cons (cons fname sub1) env))
                                          (cons (cons arg-name sub2) env))))
